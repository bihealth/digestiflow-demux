...
As the entry point Snakefile would always be the same anyway, it is much more convenient to wrap
the call to snakemake itself.
"""
...
CONFIG_FILE = "digestiflow_demux_Config.json"

#: Path to default configuration, from package (the reason why the package is
#: not zip_safe.
DEFAULT_CONFIG_YAML = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "config.yaml"))
...
def write_config_file(args):
    ...
    keys = ("input_dir", "output_dir", "lanes", "tiles", "barcode_mismatches", "cores", "continue")
    for key in keys:
        if args_dict.get(key):
            config["digestiflow_demux"][key] = args_dict[key]
    ...
    mismatches = config["sample_sheet"][0].get("bcl2fastq_args", {}).get("barcode_mismatches")
    if config["digestiflow_demux"]["barcode_mismatches"] is None and mismatches is not None:
        config["digestiflow_demux"]["barcode_mismatches"] = mismatches
    # Write out configuration as JSON.
    out_path = os.path.join(args.output_dir, CONFIG_FILE)
    print("Writing configuration+sheet to {}".format(out_path), file=sys.stderr)
...
def work(args, workdir, config_path, config):
    ...
    argv = [
        ...
        config_path,
        "--cores",
        config["digestiflow_demux"]["cores"],
        "--use-conda",
    ]
    if args.verbose:
    ...
    print('Executing "snakemake {}"...'.format(" ".join(argv)), file=sys.stderr)
    snakemake.main(argv)
...
def run(args):
    ...
    if args.work_in_output:
        print("Working in output directory: {}".format(args.output_dir), file=sys.stderr)
        work(args, args.output_dir, config_path, config)
    else:
        with tempfile.TemporaryDirectory(prefix="digestiflow-demux") as tmpdir:
            print("Working in tmp directory: {}".format(tmpdir), file=sys.stderr)
...
def main(argv=None):
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        help=("Path to input sequencer output folder, overrides setting in " "config YAML."),
    )
    group.add_argument(
        "--output-dir",
        type=str,
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        help=("Path to sample sheet YAML file, overrides setting in " "config YAML."),
    )
    group.add_argument(
        "--config",
        type=str,
        default=DEFAULT_CONFIG_YAML,
        help="Path to configuration YAML file. Default: {}".format(DEFAULT_CONFIG_YAML),
    )
    ...
    group.add_argument(
        ...
        type=int,
        default=None,
        help=("Mismatches to allow in barcode, default is 0 for v1 and " "1 for v2"),
    )
    group.add_argument(
        "--cores",
        type=int,
