...
class Library:
    ...
    def file_names(self, version, is_paired, lane=None, seq=None, name=None):
        ...
        """
        assert version in [1, 2]
        indices = [self.barcode.seq if self.barcode and self.barcode.seq else "NoIndex"]
        reads = ("R1", "R2") if is_paired else ("R1",)
        lanes = ["L{l:03d}".format(l=l) for l in self.lanes if (lane is None) or (l == lane)]
        seq = seq
        if seq is None:
            seq = ""
        else:
        ...
        return list(
            sorted(
                [
                    tpl.format(
                        sample_name=name or self.name, index=index, lane=lane, read=read, seq=seq
                    )
...
NameTokens = collections.namedtuple(
    "NameTokens", ["date", "instrument", "run", "slot", "flow_cell", "label"]
)
...
class FlowCell:
    ...
    def __init__(
        self, name, num_lanes, operator, rta_version, is_paired, read_length, libraries=[]
    ):
    ...
    def undetermined_libraries(self):
        ...
        for lane in lanes:
            result.append(
                Library(
                    "lane{}".format(lane), "none", Barcode("Undetermined", "Undetermined"), [lane]
                )
            )
        return result
...
class SampleSheet:
    ...
    def from_yaml_data(klass, data):
        ...
        for d in data["libraries"]:
            ...
            if "barcode2" in d:
                ...
                barcode2 = None
            flow_cell.libraries.append(
                Library(d["name"], d["reference"], barcode, d["lanes"], barcode2)
            )
    ...
    def print_v1(self, f):
        ...
        for lib in self.flow_cell.libraries:
            for lane in sorted(lib.lanes):
                data = [
                    self.flow_cell.name_tokens.flow_cell,
                    str(lane),
                    lib.name,
                    lib.reference,
                    lib.barcode_seq(1),
    ...
    def print_v2(self, f):
        ...
        # Write [Data] Section
        writer.writerow(["[Data]"])
        barcode2 = any(library.barcode2 for library in self.flow_cell.libraries)
        if barcode2:
        ...
        for lib in self.flow_cell.libraries:
            for lane in sorted(lib.lanes):
                ...
                print("dual = {}".format(barcode2), file=sys.stderr)
                rows.append([lane, lib.name] + list(lib.barcode_seq(2, barcode2)) + ["Project"])
...
class Bcl2FastqBaseWrapper:
    ...
    def __init__(self, config):
        ...
        self.sample_sheet = SampleSheet.from_yaml_data(config["sample_sheet"][0])
    ...
    def get_tiles_arg(self):
        ...
        demux_config = self.config["digestiflow_demux"]
        ...
        if demux_config["tiles"]:
            ...
            regexes = ["s_{}".format(lane) for lane in sorted(demux_config["lanes"])]
            return "--tiles {}".format(",".join(regexes))

    def lane_enabled(self, lane):
        """Return whether lane enabled via args"""
        if not self.config["digestiflow_demux"]["lanes"]:
            return True
        else:
            return lane in self.config["digestiflow_demux"]["lanes"]
...
class Bcl2FastqV1Wrapper(Bcl2FastqBaseWrapper):
    ...
    def barcode_mismatches(self):
        ...
        if self.config["digestiflow_demux"]["barcode_mismatches"] is None:
            return 1
        else:
            return self.config["digestiflow_demux"]["barcode_mismatches"]
    ...
    def get_output_files(self):
        ...
        for lib in flow_cell.libraries + flow_cell.undetermined_libraries:
            for lane in sorted(lib.lanes):
                ...
                output_dir = self.config["digestiflow_demux"]["output_dir"]
                ...
                for fname in lib.file_names(flow_cell.rta_version, flow_cell.is_paired, lane):
                    yield "{out_dir}/{fname}".format(out_dir=out_dir, fname=fname)
...
class Bcl2FastqV2Wrapper(Bcl2FastqBaseWrapper):
    ...
    def _build_sample_map(self):
        ...
        result = {}
        rows = [(lane, lib.name) for lib in self.flow_cell.libraries for lane in lib.lanes]
        i = 1
    ...
    def get_output_files(self):
        ...
        for lib in flow_cell.libraries + flow_cell.undetermined_libraries:
            for lane in sorted(lib.lanes):
                ...
                name = "Undetermined" if lib.barcode_seq(1) == "Undetermined" else lib.name
                for fname in lib.file_names(
                    flow_cell.rta_version, flow_cell.is_paired, lane, seq, name
                ):
                    yield "{out_dir}/{fname}".format(out_dir=out_dir, fname=fname)
