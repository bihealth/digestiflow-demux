...
for i, item in enumerate(snakemake.config["hts_screen"]["references"]):
    tee_cmd += " >(bwa_alignment {i} {ref_name} {path_index})".format(
        i=(i + 1), ref_name=item["name"].replace(" ", "_"), path_index=item["bwa_index"]
    )
    refs.append(
        "{ref_name}:{path_index}".format(
            ref_name=item["name"].replace(" ", "_"), path_index=item["bwa_index"]
        )
    )
...
""".format(
    ...
    tee_cmd=tee_cmd,
    refs=" ".join(refs),
    fastq_basename=os.path.basename(str(snakemake.input.fastq)[: -len(".fastq.gz")]),
)
...
shell(introduction + functions + generated_main + kraken_postprocessing)
